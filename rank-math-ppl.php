<?php
/**
 * Rank Math + Polylang compatibility class.
 *
 * Source: official Rank Math KB "How to Make Polylang Compatible With Rank Math"
 *         https://rankmath.com/kb/polylang-compatibility/  (code updated 2026-02-12)
 *
 * Loaded by this theme's /rank-math.php file.
 * Fixes: translatable Rank Math title/meta options (incl. homepage_title),
 *        per-language sitemaps, canonical URLs, OpenGraph alternate locales,
 *        and post-meta sync/translate/export.
 */
			...
			\add_action( "rank_math/sitemap/{$type}_content", function() use ( $type ) {
				...
				if ( 'post' === $type ) {
					...
					if ( ! empty( PLL()->options['hide_default'] ) ) {
						// The home url is of course already added.
				...
				elseif ( $post_type_obj->has_archive ) {
					...
					if ( ! wpcom_vip_get_page_by_path( $slug ) ) {
						// The post type archive in the current language is already added.
						...
						foreach ( $languages as $lang ) {
							...
							$output .= $generator->sitemap_url([
								'loc' => PLL()->links_model->switch_language_in_link( get_post_type_archive_link( $type ), $lang ),
								'mod' => $mod,
							]);
						}
					}
